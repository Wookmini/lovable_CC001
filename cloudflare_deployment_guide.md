...
## 5. 환경 변수 설정 (필수)
Supabase 등 외부 API를 사용하는 경우, Cloudflare 대시보드에서 환경 변수를 반드시 추가해야 합니다.
1. Cloudflare 대시보드의 **[Workers & Pages]** -> 해당 프로젝트 설정으로 이동합니다.
2. **[Settings]** -> **[Variables and Secrets]** (환경 변수) 메뉴로 들어갑니다.
3. 로컬의 `.env.local` 파일에 있는 `VITE_SUPABASE_URL` 및 `VITE_SUPABASE_ANON_KEY` 값을 추가하고 저장합니다.

## 6. 배포 확인
1. 빌드 및 배포 과정이 화면에 로그와 함께 표시됩니다. (환경 변수를 나중에 추가했다면 '재배포'를 눌러주세요)
2. 배포가 완료되면 화면에 **`https://[프로젝트명].pages.dev`** 형태의 무료 도메인 링크가 생성됩니다.
